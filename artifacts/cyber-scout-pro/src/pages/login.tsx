        ...
        <div className="bg-card border border-primary/30 p-8 rounded-lg shadow-2xl relative">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="font-mono text-xs tracking-wider text-muted-foreground">PASSPHRASE</FormLabel>
                    <FormControl>
                      <Input 
                        type="password" 
                        placeholder="••••••••" 
              ...
              <Button 
                type="submit" 
                className="w-full font-mono tracking-widest rounded-none border border-primary hover:bg-primary hover:text-primary-foreground bg-primary/10 text-primary transition-all duration-300"
                disabled={loginMutation.isPending}
              >
                {loginMutation.isPending ? "INITIALIZING UPLINK..." : "AUTHORIZE_"}
              </Button>
            </form>
          </Form>
          ...
          <div className="mt-6 text-center">
            <p className="text-muted-foreground font-mono text-xs">
              NO CLEARANCE?{" "}
              <Link href="/register" className="text-primary hover:underline underline-offset-4 decoration-primary/50">
                REQUEST ACCESS
              </Link>
            </p>
          </div>
